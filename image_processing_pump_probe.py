...
class StackProcessingWithShiftOnSequence:
    def __init__(self, path, image_list, minimum_position, method, rzp_spectrum_roi, back_roi):
        ...
        self.single_image_specturm = []
        # guess for minimum for shift evaluation (range-> in px-shift_on-array.py ansehen, steht bei  ca. p/m 10px)
        self.minimum_position = minimum_position
        # experimental: mit ohne shift, oder spectral vergleich im stack
        self.method = method
        self.back_stats = []
        self.shift_stats = []
    ...
    def process_single_image(self, file_name, key_back):
        ...
        #calls scipt for single picture processing, choose option by key = "back" for background substraction
        #Note: the script evaulates picture-roi for on RZP roi only.
        single_picture_processing = single_image_processing.SingleImageOneRzpProcessing(self.temporary_image_array, file_name, background_avg,
                                                                                        DataDirectory_dark, self.rzp_spectrum_roi, self.back_roi)
        if key_back == "back":
            self.single_image_specturm = single_picture_processing.process_single_image_with_back()

        elif key_back == "refback":
            self.single_image_specturm, scaling, mean_pic, mean_back = single_picture_processing.process_single_image_with_referenced_back()
            self.back_stats.append(scaling)

        else:
            self.single_image_specturm = single_picture_processing.process_single_image_without_back()

        return self.single_image_specturm
    ...
    def result_array(self):
        self.result_stack.append(self.single_image_specturm)
        return self.result_stack
    ...
    def process_stack(self, key_back):
        ...
        for x in self.image_list[1:]:
            ...
            # summierter lineout mit untergrundabzug (normiert) raus single bild und ist schon in self.result_array angehängt
            self.process_single_image(x, key_back)
            plt.figure(3)
            plt.plot(self.single_image_specturm)
            plt.title("unshifted")
            self.result_array()
        # px-shift on members of stack
    ...
    def norm_single(self):
        #on particular spectral range # ToDo wenigsten als ARgument von diesre Funktion spektralen Range eingeben
        reference_intensity = np.amax(self.result_stack[0][980:1100])
        for x in range(len(self.result_stack[1:])):
            #ToDo: range entweder raus oder hier auch wieder rein
            ratio = np.amax(self.result_stack[x][980:1100])/reference_intensity
            print("normalization signle image" , ratio)
...
key = "refback" # choose "xx" for "noback", "back" for unscaled backsubstraction, "refback" for referenced back substraction
method = "xx" # choose "nix" for nix and "norm_single" for single image normalization (to 1. picture of stack)
...
# batch process data
batch_process_data = StackProcessingWithShiftOnSequence(data_directory, data_list,  minimum_reference_x, method, data_roi, back_roi)
...
batch_process_reference = StackProcessingWithShiftOnSequence(data_directory, reference_list,minimum_reference_x, method, data_roi, back_roi)
...
# the nexafas function creates nexafs, plots and saves plot and data if you dont comment it out
